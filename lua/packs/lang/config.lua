...
function M.jdtls_setup()
  ...
  local function get_jdtls_paths()
    ...
    local java_decompiler_path = join_paths(
      paths_.data_dir,
      "nwovim",
      "packages",
    ...
    local java_decompiler_bundle = vim.split(
      vim.fn.glob(join_paths(java_decompiler_path, "server", "*.jar")),
      "\n"
    )

    if java_decompiler_bundle[1] ~= "" then
      vim.list_extend(paths.bundles, java_decompiler_bundle)
    end
  ...
  local function jdtls_setup()
    ...
    local config = {
      ...
      on_attach = function(client, bufnr)
        ...
        require("lsp.protocol_handlers").setup()
...
function M.tsnode_marker_setup()
  vim.api.nvim_create_autocmd("FileType", {
